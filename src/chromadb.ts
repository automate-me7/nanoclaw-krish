...
export async function initChromaDB(): Promise<void> {
    ...
    try {
        ...
        initialized = true;
        logger.info({ url: chromaBaseUrl, collection: COLLECTION_NAME }, 'ChromaDB initialized');
    } catch (err) {
        logger.error(
...
): Promise<Array<{ document: string; metadata: Record<string, string>; distance: number }>> {
    if (!initialized) return [];
